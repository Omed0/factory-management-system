...
├── app.config.ts      ← vinxi/TanStack Start config (MUST be at root)
...
# Deploy
# VPS:  bun run supabase:prod:up
# CF:   DEPLOY_TARGET=cloudflare bun run build && bun run deploy:cf
```
...
- ❌ Moving `app.config.ts` or `components.json` out of the repo root — both tools require root placement

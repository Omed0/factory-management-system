...
function DollarPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className="text-2xl font-bold tracking-tight">Exchange Rate</h1>
        <p className="text-sm text-muted-foreground mt-0.5">IQD per 1 USD</p>
      </div>

      <Card>
        <CardHeader><CardTitle>Current Rate</CardTitle></CardHeader>
          ...
          <div className="flex items-baseline gap-3">
            ...
            {diff !== null && (
              <span className={`flex items-center gap-0.5 text-sm font-medium ${diff >= 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}`}>
                ...
                {Math.abs(diff).toLocaleString()} from last change
              </span>
            )}
          </div>
          <p className="text-xs text-muted-foreground">
            Last updated: {current.data?.updated_at ? new Date(current.data.updated_at).toLocaleString() : '—'}
          </p>

          {canWrite && (
            <form className="flex gap-2 items-end pt-2" onSubmit={(e) => { e.preventDefault(); form.handleSubmit() }}>
              <div className="flex-1">
                <TextField form={form} name="price" label="New rate (IQD per 1 USD)" type="number" required />
              </div>
              <Button type="submit" disabled={form.state.isSubmitting}>Update</Button>
            </form>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader><CardTitle>History (last 30 changes)</CardTitle></CardHeader>
        <CardContent>

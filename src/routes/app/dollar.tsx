...
const update = createServerFn({ method: "POST" })
  .inputValidator((d: unknown) =>
    z.object({ price: z.coerce.number().positive() }).parse(d),
  )
  .handler(async ({ data }) => {
    const sb = getSupabaseServer();
    ...
    if (!ok)
      throw new Error("You do not have permission to update the exchange rate");
...
function DollarPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("dollar.currentRate")}</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-baseline gap-3">
            ...
            {diff !== null && (
              <span
                className={`flex items-center gap-0.5 text-sm font-medium ${diff >= 0 ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"}`}
              >
                {diff >= 0 ? (
          ...
          <p className="text-xs text-muted-foreground">
            {current.data?.updated_at
              ? new Date(current.data.updated_at).toLocaleString()
              : "—"}
          </p>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("dollar.rateHistory")}</CardTitle>
        </CardHeader>
        <CardContent>
          {historyData.length === 0 ? (
            <p className="text-sm text-muted-foreground py-4 text-center">
              {t("dollar.noDollarHistory")}
            </p>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {delta !== null && (
                        <span
                          className={`text-xs ${delta >= 0 ? "text-red-500" : "text-green-500"}`}
                        >
                          {delta >= 0 ? "+" : ""}

...
const bootstrap = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const catalogPerms = ((catalog ?? []) as any[]).map(
    (c) => `${c.resource}:${c.action}`,
  );
  const catalogCount = catalogPerms.length;

  let permissions: string[];
  if (profile.role === "OWNER") {
    permissions = ownerPermissions(catalogPerms);
  } else if (profile.role === "ADMIN") {
    permissions = adminPermissions(catalogPerms);
  } else {
    const { data: grants } = await (sb.from("user_permissions") as any)
      .select("resource, action")
      .eq("profile_id", user.id);
    permissions = ((grants ?? []) as any[]).map(
...
function AppLayout() {
  ...
  const visibleNav = NAV.filter(
    (item) =>
      ...
      can(
        permissions,
        ...((item.perm as string).split(":") as [string, string]),
      ),
  );
          ...
          {collapsed && !isMobile ? (
            <div className="flex flex-col items-center gap-1">
              ...
              <Tooltip>
                ...
                <TooltipContent side="right">
                  {t("nav.signOutUser", { name: profile?.name })}
                </TooltipContent>
              </Tooltip>
            </div>
          ) : (

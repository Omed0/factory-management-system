...
function RootComponent() {
  ...
  return (
    <html lang={settings?.language ?? 'ckb'} dir={settings?.direction ?? 'rtl'}>
      <head>
    ...
    <div className="flex min-h-screen w-full items-center justify-center p-4 bg-background">
      <div className="w-full max-w-md rounded-xl border bg-card text-card-foreground shadow">
        <div className="flex flex-col space-y-1.5 p-6 pb-4">
          ...
          <p className="text-sm text-muted-foreground">The application encountered an issue while processing your request.</p>
        </div>
        <div className="p-6 pt-0">
          <pre className="mt-2 max-h-[250px] overflow-auto rounded-lg border bg-muted p-4 text-sm text-muted-foreground">
  ...
  return (
    <div className="flex min-h-screen w-full items-center justify-center p-4 bg-background">
      <div className="w-full max-w-md rounded-xl border bg-card text-card-foreground shadow p-8 text-center flex flex-col items-center">
        ...
        <h3 className="font-semibold tracking-tight text-2xl mb-2">Page not found</h3>
        <p className="text-sm text-muted-foreground mb-8">
          The page you are looking for does not exist or has been moved.
        </p>
        <button onClick={() => window.history.back()} className="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2 w-full max-w-[200px]">
          Go Back
        </button>
      </div>
    </div>

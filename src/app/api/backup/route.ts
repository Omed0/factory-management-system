...
const createZip = (filePath: string, outputZipPath: string): Promise<string> => {
...
const splitFile = (filePath: string, maxSize = 49 * 1024 * 1024): Promise<string[]> => {
    return new Promise((resolve, reject) => {
        ...
        readStream.on('data', (chunk) => {
            currentChunk = Buffer.concat([currentChunk, Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk)]);

            if (currentChunk.length >= maxSize) {
                const chunkFilePath = path.join(os.tmpdir(), `backup_part${chunkIndex}.zip`);
        ...
        readStream.on('error', (err) => reject(`Error reading file: ${err.message}`));
    });
};
...
export async function POST(req: Request) {
    try {
        ...
        const message = isUploadToDrive ? "بەسەرکەوتووی باکئەپ کرا بۆ کۆمپیوتەرەکە" : "بەسەرکەوتووی باکئەپ کرا بۆ تێلەگرام"
        ...
        return NextResponse.json({ description: error.message, message: 'هەڵەیەک ڕوویدا' });
    }
}

...
import {
    ...
    CreatePaidLoanSale,
    createPaidLoanSaleSchema, CreateProductSale, createProductSaleSchema,
    CreateSale, createSaleSchema, deletePaidLoanSaleSchema,
    deleteSaleSchema, discountSaleSchema, finishSaleSchema, getListPaidLoanSaleSchema,
    getListProductSaleSchema, getListSaleSchema, getOneSaleSchema, restoreSaleSchema,
...
async function getCustomerById(id: number, tx: Prisma.TransactionClient) {
    const customer = await tx.customers.findFirst({ where: { id, deleted_at: null } })
    if (!customer) throw new Error("کڕیار نەدۆزرایەوە")
    return { customer }
}

async function getSalesByCustomerId(customerId: number, tx: Prisma.TransactionClient) {
    const sales = await tx.sales.findMany({ where: { customerId, customer: { deleted_at: null } } })
    if (!sales) throw new Error("وەصڵ نەدۆزرایەوە")
    if (!sales.length) throw new Error("ئەم کەسە وەصڵی نییە")
    return { sales }
}

async function getSaleById({ id, customerId }: { id: number, customerId: number }, tx: Prisma.TransactionClient) {
...
async function getPaidLoansBySaleId(saleId: number, tx: Prisma.TransactionClient) {
    const loan = await tx.paidLoans.findMany({ where: { saleId, sale: { deleted_at: null } } })
...
export async function getCustomerOneSale({ id, customerId }: { id: number, customerId: number }) {
    return tryCatch(async () => {
        const data = getOneSaleSchema.parse({ id, customerId });
        const result = await prisma.$transaction(async (tx) => {
            const [customer, sale] = await Promise.all([getCustomerById(data.customerId, tx),
...
export async function getCustomerListSale({ customerId, isTrash }: { customerId: number, isTrash: boolean }) {
    return tryCatch(async () => {
        const data = getListSaleSchema.parse({ customerId });
        const sales = await prisma.sales.findMany({
            where: { customerId: data.customerId, deleted_at: isTrash ? { not: null } : null },
...
export async function createSaleForCustomer({ saleValues }: { saleValues: CreateSale }) {
    return tryCatch(async () => {
...
export async function updateSaleForCustomer({ saleValues }: { saleValues: UpdateSale }) {
    return tryCatch(async () => {
        const { id: saleId, ...rest } = updateSaleSchema.parse({ ...saleValues });

        const sale = await prisma.$transaction(async (tx) => {
            const { customer } = await getCustomerById(rest.customerId, tx)
            const { sale } = await getSaleById({ id: saleId, customerId: customer.id }, tx)

            if (sale.saleType !== rest.saleType) throw new Error("گۆڕانکاری ناکرێت لە جۆری پارەدان، بەردەوامبە لەسەر شێوازی پێشوو")
...
export async function deleteSaleForCustomer({ id, customerId }: { id: number, customerId: number }) {
    return tryCatch(async () => {
        const data = deleteSaleSchema.parse({ id, customerId });
        const sale = await prisma.sales.update({
            where: {
                id: data.id, customerId: data.customerId,
                deleted_at: null, customer: { deleted_at: null }
            },
...
export async function restoreSaleForCustomer({ id, customerId }: { id: number, customerId: number }) {
    return tryCatch(async () => {
...
export async function forceDeleteSaleForCustomer({ id, customerId }: { id: number, customerId: number }) {
    return tryCatch(async () => {
        const data = restoreSaleSchema.parse({ id, customerId });
...
export async function discountForSaleInvoice({ id, discount }: { id: number, discount: number }) {
    return tryCatch(async () => {
        const data = discountSaleSchema.parse({ saleId: id, discount })
        const sale = await prisma.sales.update({
            where: { id: data.saleId, deleted_at: null, isFinished: false, totalAmount: { gte: data.discount } },
...
export async function finishSaleInvoice({ id, isFinished }: { id: number, isFinished: boolean }) {
    return tryCatch(async () => {
        ...
        if (isCash) {
            const sale = await prisma.sales.update({
                ...
                data: { isFinished: data.isFinished, totalRemaining: currentSale.totalAmount }
            })
            await prisma.boxes.update({
                where: { id: 1 },
                data: { amount: { increment: (currentSale.totalAmount - currentSale.discount) } }
            })
...
export async function getProductSaleList({ saleId, customerId }: { saleId: number, customerId: number }) {
    return tryCatch(async () => {
        ...
        const sale = await prisma.sales.findFirst({
            where: {
                id: data.saleId,
                customerId: data.customerId, isFinished: false,
                deleted_at: null, customer: { deleted_at: null }
            },
        ...
        const productSale = await prisma.saleItems.findMany({ where: { saleId: sale.id } });

        const productIds = new Set(productSale.map(item => item.productId).filter((id): id is number => id !== null));
        ...
        if (!allProductsWithSameId) throw new Error('مەوادەکانی ناو ئەم وەصڵە نەدۆزرانەوە');

        return { productSale: allProductsWithSameId, sale };
    })
}

export async function createProductSaleList({ product }: { product: CreateProductSale }) {
    return tryCatch(async () => {
        ...
        const newProduct = await prisma.$transaction(async (tx) => {
            ...
            if (existingProduct) {
                ...
                return increaseQuantitySaleItem({ id: existingProduct.id, amount: data.quantity });
            } else {
                // If it doesn't exist, create a new product sale
                const saleItem = await tx.saleItems.create({ data });
                await tx.sales.update({
                    where: { id: saleItem.saleId },
                    data: { totalAmount: { increment: saleItem.price * saleItem.quantity } }
                });
...
export async function increaseQuantitySaleItem({ id, amount }: { id: number, amount: number }) {
    return tryCatch(async () => {
        ...
        const updatedProduct = await prisma.$transaction(async (tx) => {
            ...
            await tx.sales.update({
                ...
                data: { totalAmount: { increment: updateItemInSale.price * data.amount } }
            });

            if (!updateItemInSale) throw new Error('هەڵەیەک ڕوویدا لە زیادکردنی مەواد');
...
export async function decreaseQuantitySaleItem({ id, amount }: { id: number, amount: number }) {
    return tryCatch(async () => {
        const data = changeProductQuantitySchema.parse({ id, amount });
        const currentProduct = await prisma.$transaction(async (tx) => {
            const findSaleItem = await tx.saleItems.findUnique({ where: { id: data.id } });
            ...
            if (findSaleItem) {
                if (findSaleItem.quantity === 1) {
                    ...
                    if (!updatedProduct) throw new Error('هەڵەیەک ڕوویدا لە کەمکردنەوەی دانە');
                    await tx.sales.update({
    ...
    return tryCatch(async () => {
        const deletedProduct = await prisma.$transaction(async (tx) => {
            ...
            await tx.sales.update({
                ...
                data: {
                    totalAmount: { decrement: saleItemProduct.price * saleItemProduct.quantity }
                }
            });
...
export async function getPaidLoanSaleList({ saleId }: { saleId: number }) {
    return tryCatch(async () => {
        ...
        const sale = await prisma.sales.findFirst({ where: { id: data.saleId, deleted_at: null } })
        if (!sale) throw new Error("ئەم وەصڵە نەدۆزرایەوە")

        const loan = await prisma.paidLoans.findMany({ where: { saleId: sale.id } })
...
export async function createPaidLoanSaleList({ paidLoanInfo }: { paidLoanInfo: CreatePaidLoanSale }) {
    return tryCatch(async () => {
        ...
        const loan = await prisma.$transaction(async (tx) => {
            ...
            const adjustedTotalAmount = (totalAmount._sum.amount || 0) - (sale?.discount || 0);
            const isLessThanTotalAmount = adjustedTotalAmount + data.amount;

            await tx.sales.update({
                where: { id: data.saleId, deleted_at: null, totalAmount: { gte: isLessThanTotalAmount } },
...
export async function deletePaidLoanSaleList({ id, saleId }: { id: number, saleId: number }) {
    return tryCatch(async () => {
        const data = deletePaidLoanSaleSchema.parse({ id, saleId });
        const loan = await prisma.$transaction(async (tx) => {
            const deletedLoan = await tx.paidLoans.delete({ where: { id: data.id, saleId: data.saleId } })

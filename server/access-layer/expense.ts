...
export async function deleteManyExpenses(ids: number[]) {
  return tryCatch(async () => {
    ...
    const { expenses, box } = await prisma.$transaction(async (tx) => {
      ...
      const totalAmount = expensesToDelete.reduce(
...
export async function restoreManyExpenses(ids: number[]) {
  return tryCatch(async () => {
    ...
    const { expenses, box } = await prisma.$transaction(async (tx) => {
      ...
      const totalAmount = expensesToRestore.reduce(
        (acc, curr) => acc + curr.amount,
        0
      );
...
export async function getExpensesListActionsSpecificTime(
  startOfMonth: Date,
  endOfMonth: Date
) {
  return tryCatch(async () => {
    const data = getExpensesListActionsSpecificTimeSchema.parse({
      startOfMonth,
      endOfMonth,
    });
